...
<html lang="el">
  ...
  <body>
    ...
    <img class="forum-foto" src="./images/forest.jpg">
    ...
    <div class="row">
      <div class="leftcolumn">
        ...
        if(isset($_GET["categoryID"]))
        {
          ...
          $categoryNameQuery = "SELECT Όνομα FROM category WHERE ID = $categoryID";
          $categoryNameRes = mysqli_query($conn, $categoryNameQuery);
          $categoryName = mysqli_fetch_assoc($categoryNameRes);
  
          echo '<div class="card" style="color:#4f9e6e; margin-bottom: 50px"><div class="topic-title">' . $categoryName["Όνομα"] . '</div></div>';  
        }
        elseif(isset($_GET["search"]))
        {
          $search = $_GET["search"];
          echo '<div class="card" style="color:#4f9e6e; margin-bottom: 50px"><div class="topic-title">Αποτελέσματα Αναζήτησης</div></div>';
        }
        else
        {
          $userID = intval($_SESSION["ID"]);
          echo '<div class="card" style="color:#4f9e6e; margin-bottom: 50px"><div class="topic-title">Οι Συζητήσεις Μου</div></div>';
        }
        ?>
        ...
        if(isset($_GET["categoryID"]))
        {
          $numOfTopicsQuery = "SELECT COUNT(*) FROM topic WHERE CategoryID = $categoryID";
          $numOfTopicsResult = mysqli_query($conn, $numOfTopicsQuery);
          $numOfTopics = mysqli_fetch_assoc($numOfTopicsResult)["COUNT(*)"];
          $totalPages = ceil($numOfTopics / $limit);

          $topicsQuery = "SELECT * FROM topic WHERE CategoryID = $categoryID LIMIT $offset, $limit";
          $topics = mysqli_query($conn, $topicsQuery);
        }
        elseif(isset($_GET["search"]))
        {
          $numOfTopicsQuery = "SELECT COUNT(*) FROM topic WHERE Τίτλος LIKE '%$search%' OR Περιγραφή LIKE '%$search%' OR Κείμενο LIKE '%$search%'";
          $numOfTopicsResult = mysqli_query($conn, $numOfTopicsQuery);
          $numOfTopics = mysqli_fetch_assoc($numOfTopicsResult)["COUNT(*)"];
          $totalPages = ceil($numOfTopics / $limit);

          $topicsQuery = "SELECT * FROM topic WHERE Τίτλος LIKE '%$search%' OR Περιγραφή LIKE '%$search%' OR Κείμενο LIKE '%$search%' LIMIT $offset, $limit";
          $topics = mysqli_query($conn, $topicsQuery);
        }
        else
        {
          $numOfTopicsQuery = "SELECT COUNT(*) FROM topic WHERE UserID = $userID";
          $numOfTopicsResult = mysqli_query($conn, $numOfTopicsQuery);
          $numOfTopics = mysqli_fetch_assoc($numOfTopicsResult)["COUNT(*)"];
          $totalPages = ceil($numOfTopics / $limit);

          $topicsQuery = "SELECT * FROM topic WHERE UserID = $userID LIMIT $offset, $limit";
          $topics = mysqli_query($conn, $topicsQuery);
        }

        if($pageNo > $totalPages || $pageNo < 1)
        {
          echo '<div class="card" style="color:#4f9e6e"><div class="topic-title">Η σελίδα δεν βρέθηκε</div></div>';
        }
        elseif($numOfTopics > 0)
        {
          while($topic = mysqli_fetch_assoc($topics))
          {
            echo 
            '<div class="card">
              <a href="profile.php?UserID=' . $topic["UserID"] . '"><img class="avatar" src="./images/profileicon.jpg"></a>
              <div class="topic">
                <div class="topic-title"><a href="TopicName.php?topicID=' . $topic["ID"] . '">' . $topic["Τίτλος"] . '</a></div>';

                  $dateArr = explode('-', $topic["Ημερομηνία"]);
                  $date = (int) $dateArr[2] . " " . $months[(int) $dateArr[1] - 1] . " " . $dateArr[0];
                  
                echo 
                '<div class="description">' . $topic["Περιγραφή"] . ', ' . $date . '</div>
                <p>' . substr($topic["Κείμενο"], 0, 30) . '...' . '</p>
              </div>';

              $numOfPostsQuery = "SELECT COUNT(*) FROM post WHERE TopicID = " . $topic["ID"];
              $numOfPostsQueryResult = mysqli_query($conn, $numOfPostsQuery);
              $numOfPosts = mysqli_fetch_assoc($numOfPostsQueryResult);

              echo '
              <div class="count-cell">
                Απαντήσεις: ' . $numOfPosts["COUNT(*)"] .'
              </div>
              <div class="count-cell">
                Προβολές: ' . $topic["Προβολές"] . '
              </div>';
              ...
              if($userType["Τύπος"] == "Administrator")
              {
                echo '
                <div class="delete-topic" onClick="openDeleteTopicDialog(' . $topic["ID"] . ')">
                  &times;
                </div> ';
              }
            echo '</div>';
        ...
        {
          echo '<div class="card">Η κατηγορία δεν υπάρχει</div>';
        }
        ?>            

        <div class="delete-topic-dialog" id="delete-topic-dialog">
          <div class="delete-topic-dialog-content">
            <span  class="close-dialog" onClick="closeDialog()">&times;</span>
            <br>
            <div style="text-align: center;">Είστε σίγουροι ότι θέλετε να διαγράψετε αυτήν την συζήτηση και όλες τις αναρτήσεις που της αντιστοιχούν;</div>
            <form method="POST" action="
            <?php
            if(isset($_POST["delete-topic-button"]))
            {
              $deletePostsQuery = "DELETE FROM post WHERE topicID = " . $_POST["input-topicID"];
              $deleteTopicQuery = "DELETE FROM topic WHERE ID = " . $_POST["input-topicID"];
              $deletePostsRes = mysqli_query($conn, $deletePostsQuery);
              $deleteTopicRes = mysqli_query($conn, $deleteTopicQuery);
              if($deleteTopicRes && $deletePostsRes)
              {
                ob_clean();
                echo "Η συζήτηση διαγράφηκε";
                $redirect = "Location: Category.php?categoryID=" . $GLOBALS['categoryID'];
                header($redirect);
                exit();
              }
              else
            ...
            ?>">
              <input type="hidden" name="input-topicID" id="input-topicID"></input>
              <button class="delete-topic-button" id="delete-topic-button" name="delete-topic-button" type="submit">
                Διαγραφή Συζήτησης
              </button>
            </form>
          </div>
        ...
        <div class="centerpagination">
          <div class="pagination">
            ...
            ?>
            <a href="<?php echo $temp . 'pageNo=1'?>">&laquo; Πρώτη Σελίδα</a>
            <a href="<?php echo $temp; echo ($pageNo <= 1) ? '#' : "pageNo=".($pageNo - 1); ?>" class="<?php if($pageNo <= 1){ echo 'disabled'; } ?>">&lsaquo;</a>
            <?php
              for($i = 1; $i <= $totalPages; $i++)
              {
                echo '<a href="' . $temp . 'pageNo=' . $i .'"';
                echo ' class=';
                echo ($pageNo == $i)?"active":"";
                echo '>' . $i . '</a>';
              }
            ?>
            <a href="<?php echo $temp; echo ($pageNo >= $totalPages) ? 'pageNo='. $totalPages : "pageNo=".($pageNo + 1); ?>" class="<?php if($pageNo >= $totalPages){ echo 'disabled'; } ?>">&rsaquo;</a>
            <a href="<?php echo $temp . 'pageNo=' . $totalPages;?>">Τελευταία Σελίδα &raquo;</a>
          </div>  
        </div>
