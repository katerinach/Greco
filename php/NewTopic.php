...
<html lang="el">
  ...
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="./stylesheets/NewTopic.css">
    <title>GReco | New Topic</title>
  </head>
  ...
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
    <div class="row">
      <div class="leftcolumn">
        ...
        <div class="post">
          <h2 style="padding-bottom: 10px; border-bottom: #d0d0d0 1px solid; margin-top: 0px;">Νέα Συζήτηση</h2>
          <div class="field">
            <div class="field-name">Θέμα Συζήτησης</div>
            <textarea name="topictitle" id="topictitle" required style="width: 100%; height: 30px; resize: vertical;"></textarea>  
          </div>
          <div class="field">
            <div class="field-name">Κατηγορία</div>
            <select name="category" id="category">
              ...
              if(mysqli_num_rows($categories) > 0)
              {
                ...
                {
                  echo '<option value="' . $category["ID"] . '">' . $category["Όνομα"] . '</option>';
                }
              }
              ?>
            </select>
          ...
          <div class="field">
            <div class="field-name">Περιγραφή</div>
            <textarea name="description" id="description" required style="width: 100%; height: 50px; resize: vertical;"></textarea>  
          </div>
          <div class="field">
            <div class="field-name">Κείμενο</div>
            <textarea name="message" id="message" required style="width: 100%; height: 100px; resize: vertical;"></textarea>  
          </div>

          <button class="newtopic" type="submit">
          ...
          {
              $topictitle = test_input($_POST["topictitle"]);
              $categoryID = test_input($_POST["category"]);
              $description = test_input($_POST["description"]);
              $message = test_input($_POST["message"]);
              $date = date("Y-m-d");
      
              $insertQuery = "INSERT INTO topic (ID, Τίτλος, Περιγραφή, Κείμενο, Ημερομηνία, CategoryID, UserID, Προβολές) 
              VALUES (NULL, '$topictitle', '$description', '$message', '$date', '$categoryID ', '" . $_SESSION["ID"] . "', '0')";
      
              if(mysqli_query($conn, $insertQuery))
              {
                $newTopicIDQuery = "SELECT ID FROM topic WHERE Τίτλος = '" . $topictitle . "'";
                $newTopicIDRes = mysqli_query($conn, $newTopicIDQuery);
                $newTopicID = mysqli_fetch_assoc($newTopicIDRes);
                echo '<br>Η Συζήτηση Αποθηκεύτηκε';
                header("Location: TopicName.php?topicID=".$newTopicID["ID"]);
              }
              else
              {
                echo '<br>Error';
